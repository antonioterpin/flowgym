"""Script to replicate experiment 3.4 in # TODO: add link to paper.
...
def run_sweep():
    ...
    for combo in COMBOS:
        ...
        for val in TAU_VALUES:
            ...
            regularized = (
                "reg" if any(v > 0.0 for v in regularizer_weights.values()) else "noreg"
            )
            # Write temp config
            temp_path = CONFIG_PATH.parent / f"temp_{regularized}_{flag}_{val}.yaml"
