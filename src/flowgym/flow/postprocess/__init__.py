...
def quantize_validate_params(
    ...
    """
    if not isinstance(min_val, jnp.ndarray) or not isinstance(
        max_val, jnp.ndarray
    ):
...
def resize_flow_validate_params(
    ...
    if target_height <= 0 or target_width <= 0:
        raise ValueError(
            "target_height and target_width must be positive integers."
        )
...
def validate_params(
    ...
    **kwargs,
    ...
    Args:
        postprocessing_step_name: Name of the postprocessing step.
        **kwargs: Parameters for the postprocessing step.

    Raises:
        ValueError: If the postprocessing step is unknown.
    """
    ...
    if validate_func_name in globals():
        ...
        raise ValueError(
            f"Unknown postprocessing step {postprocessing_step_name}"
        )
