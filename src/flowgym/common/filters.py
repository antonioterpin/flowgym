...
def gaussian_kernel(
    sigma: float, truncate: float, n_channels: int
) -> jnp.ndarray:
    ...
    if DEBUG:
        ...
        assert isinstance(n_channels, int), (
            "Number of channels must be an integer."
        )
...
def gaussian_smoothing(
    ...
    if DEBUG:
        ...
        assert data.ndim == 4, (
            "Data must be 4D (batch_size, height, width, channels)."
        )
        ...
        assert data.shape[1] > kernel.shape[0], (
            "Data height must be greater than kernel height."
        )
        assert data.shape[2] > kernel.shape[1], (
...
def sobel() -> tuple[jnp.ndarray, jnp.ndarray]:
    ...
    Returns:
        Tuple `(kx, ky)` with Sobel kernels for x and y gradients.
    """
