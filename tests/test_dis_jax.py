...
EXECUTIONS_EXTRACT_PATCHES_GRAD_HESS = config[
    "EXECUTIONS_EXTRACT_PATCHES_GRAD_HESS"
]
...
def test_flow_between(test_images, visualize=True):
    ...
    flow = flow_between(
        ...
        starting_flow=jnp.zeros(
            (prev.shape[0], prev.shape[1], 2), dtype=jnp.float32
        ),
        var_refine_iters=0,
    )
    ...
    assert counter < 0.05 * cropped_flow.size, (
...
def test_images(request):
    ...
    A larger image is generated and the first image is generated by
    sampling from
    the center of the larger image. The second image is generated by
    sampling from
    the larger image with the opposite of the specified shift using the function
    sample_patch. It only works for square images.

    Args:
        request: pytest fixture request object containing parameters for
            the test.

    Returns:
        tuple: A tuple containing the first image, the second image, and
            the shift.
    """
    ...
    larger_size = (
        image_size[0] + bounds[0],
        image_size[1] + bounds[1],
    )  # (H, W)
    ...
    center = jnp.array(
        [
            bounds[0] // 2 + image_size[0] // 2,
            bounds[1] // 2 + image_size[1] // 2,
        ]
    )
...
def test_output_shape(test_images, patch_size, patch_stride, grad_iters):
    ...
    assert flow.shape == starting_flow.shape, (
        "Output shape must be (num_patches, 2)"
    )
...
@pytest.mark.parametrize(
    "test_images",
    [{"shift": (0.0, 0.0), "image_size": (128, 128)}],
    indirect=True,
)
def test_zero_flow_identity(patch_size, patch_stride, grad_iters, test_images):
    ...
    assert jnp.allclose(flow, 0, atol=1), (
...
def test_img_resize(image_size, new_size):
    ...
    assert jnp.allclose(resized_img, gound_truth, atol=1e-5), (
...
def test_speed_flow_between(
    ...
    curr = warp_image(prev, jnp.array(disp), 1)
    starting_flow = jnp.zeros(
        (image_size[0], image_size[1], 2), dtype=jnp.float32
    )
...
def test_speed_estimate_dis_flow(
    ...
    # Create a random image
    key = random.PRNGKey(0)
    prev_batch = random.uniform(
        key, shape=(batch_size, image_size[0], image_size[1])
    )
    ...
    def run_jit_fn():
        flow_batch = estimate_dis_flow_jit(
            prev_batch, curr_batch, starting_flow
        )
...
def test_speed_densify(
    ...
    patch_errors = random.uniform(
        key, shape=(num_patches, patch_size, patch_size)
    )
...
def test_speed_extract_patches_grad_hess(
    ...
    # Average time
    average_time_jit = (
        min(total_time_jit) / EXECUTIONS_EXTRACT_PATCHES_GRAD_HESS
    )
...
def test_speed_query_flow_at_points(
    ...
    prev = random.uniform(
        subkey1,
        shape=(
            image_shape[0] // (2**next_level),
            image_shape[1] // (2**next_level),
        ),
    )
...
def test_error_vs_opencv_dis():
    ...
    flow = jnp.ones((shape[0], shape[1], 2), dtype=jnp.float32) * jnp.array(
        [1, 1]
    )
    ...
    dis_jax_flow = flow_between(
        ...
        output_full_res=True,
        starting_flow=jnp.zeros(
            (img.shape[0], img.shape[1], 2), dtype=jnp.float32
        ),
        var_refine_iters=0,
    )
    ...
    error_wrt_gt_dis_jax = jnp.mean(
        jnp.linalg.norm(
            dis_jax_flow[17:-17, 17:-17] - flow[17:-17, 17:-17], axis=-1
        )
    )
    ...
    error = jnp.mean(
        jnp.linalg.norm(
            dis_jax_flow[17:-17, 17:-17] - cv_flow[17:-17, 17:-17], axis=-1
        )
    )
